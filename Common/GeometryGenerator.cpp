...
GeometryGenerator::MeshData GeometryGenerator::CreateGrid(float width, float height, float division)
{
	MeshData meshData;
	...
	for (float x = -w2; x <= w2; x += division)
	{
	...
	for (float y = -h2; y <= h2; y += division)
	{
		XMVECTORF32 color = index % 10 == 0 ? Colors::Black : Colors::Gray;
	...
	meshData.Indices32.assign(i.begin(), i.end());
