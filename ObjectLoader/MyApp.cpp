...
void MyApp::UpdateObjectCBs(const GameTimer& gt)
{
	...
	for (int i = 0; i < mAllRitems.size(); i++)
	{
		...
		if (mAllRitems[i]->NumFramesDirty > 0)
		{
			...
			//4th exercise
			/*XMMATRIX texTransform = XMMATRIX(cos(gt.TotalTime()), sin(gt.TotalTime()), .0f, .0f, cos(gt.TotalTime()), -sin(gt.TotalTime()) , .0f, .0f, .0f, .0f,
				.0f, .0f, .0f, .0f, .0f, .0f);
			XMStoreFloat4x4(&objConstants.TexTransform, XMMatrixTranspose(texTransform));*/
			/*objConstants.TexTransform = XMFLOAT4X4(cos(gt.TotalTime()), sin(gt.TotalTime()), .0f, .0f, cos(gt.TotalTime()), -sin(gt.TotalTime()), .0f, .0f, .0f, .0f,
				.0f, .0f, .0f, .0f, .0f, .0f);*/
...
void MyApp::BuildRenderItems()
{
	...
	modelRitem->Geo = mGeometries[L"modelGeo"].get();
	modelRitem->PrimitiveType = D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST;
	modelRitem->IndexCount = modelRitem->Geo->DrawArgs[L"model"].IndexCount;
	modelRitem->StartIndexLocation = modelRitem->Geo->DrawArgs[L"model"].StartIndexLocation;
	modelRitem->BaseVertexLocation = modelRitem->Geo->DrawArgs[L"model"].BaseVertexLocation;
...
void MyApp::addObjectControl(const std::wstring& name)
{
	...
	_selectedObject = (int)_objectBtns.size() - 1;
	showTransform(true);
